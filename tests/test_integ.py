...
def test_full_login_flow_with_valid_email(client):
    ...
    decoded = html.unescape(response_summary.get_data(as_text=True))
    assert clubs[0]["name"] in decoded
    assert "competitions" in decoded.lower() 
...
def test_full_booking_flow_with_save(monkeypatch, client):
    ...
    assert int(saved_data["competitions"][0]["numberOfPlaces"]) == original_places - 5


def test_overbooking_flow(client):
    c, clubs, competitions = client

    response_overbook = c.post(
        "/purchasePlaces",
        data={
            "club": clubs[0]["name"],
            "competition": competitions[0]["name"],
    ...
    decoded = html.unescape(response_overbook.get_data(as_text=True))
    assert "pour réserver autant de places" in decoded 
